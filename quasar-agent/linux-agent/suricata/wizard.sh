...
# Function to run Docker container with Wazuh and Suricata
run_docker_container() {
    ...
    # Prompt the user if they want to install Suricata
    read -p "Do you want to include Suricata in this setup? (y/n): " INSTALL_SURICATA

    # Set the default Docker image (this is the placeholder that will be replaced)
    DOCKER_IMAGE="your_docker_image"

    # Call the function to verify if the Docker image exists or if it's still the placeholder
    verify_docker_image

    # Build the Docker run command based on user input
    DOCKER_CMD="docker run -e WAZUH_MANAGER=$WAZUH_MANAGER -e WAZUH_AGENT_NAME=$WAZUH_AGENT_NAME $DOCKER_IMAGE"

    # If user chooses to install Suricata, add the "-s" option to the Docker command
    if [[ "$INSTALL_SURICATA" == "y" || "$INSTALL_SURICATA" == "Y" ]]; then
        DOCKER_CMD="$DOCKER_CMD -s"
    fi

    # Display the final command that will be executed
...
verify_docker_image() {
    if [[ "$DOCKER_IMAGE" == "your_docker_image" ]]; then
        ...
        read -p "Please enter the actual Docker image name (e.g., wazuh-suricata-image): " DOCKER_IMAGE
    fi
...
while true; do
    ...
    case $MAIN_CHOICE in
        ...
        3)
            while true; do
                show_wazuh_menu
                read -p "Select Wazuh Operation (1-3): " WAZUH_CHOICE
                case $WAZUH_CHOICE in
                    1)
                        run_docker_container "wazuh"
                        ;;
                    2)
                        run_docker_container "wazuh_suricata"
                        ;;
                    3)
                        break
                        ;;
